...
app.post("/api/accommodations", async (req, res) => {
  try {
    const { fullName, email, phone, checkInDate, checkOutDate, numberOfPeople, roomType } = req.body;

    if (!fullName || !email || !phone || !checkInDate || !checkOutDate || !roomType) {
    ...
    res.status(201).json({ message: "Accommodation booked successfully", id: result.insertedId });
  } catch (err) {
...
app.get("/api/donations", verifyAdminToken, async (req, res) => {
  try {
...
app.get("/api/donation-usage", verifyAdminToken, async (req, res) => {
  try {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      bookings.forEach((booking) => {
        ...
        if (date >= checkIn && date < checkOut) {
          isBooked = true; // Fully booked
        } else if (
          date.getTime() === checkIn.getTime() ||
          date.getTime() === checkOut.getTime()
        ) {
          isFilling = true; // Partially booked
        }
      });
...
// Modify the POST /api/accommodations to set status as pending
app.post("/api/accommodations", async (req, res) => {
  try {
    const {
      fullName,
